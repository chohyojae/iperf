...
iperf_tcp_accept(struct iperf_test * test)
{
    ...
    len = sizeof(addr);
    if ((s = ff_accept(test->listener, (struct sockaddr *) &addr, &len)) < 0) {
...
iperf_tcp_connect(struct iperf_test *test)
{
    ...
    if (test->bind_address) {
        ...
        if (ff_bind(s, (struct sockaddr *) local_res->ai_addr, local_res->ai_addrlen) < 0) {
	    saved_errno = errno;
	    ff_close(s);
	    freeaddrinfo(local_res);
	    freeaddrinfo(server_res);
	...
	/* Unknown protocol */
        ...
        if (ff_bind(s, (struct sockaddr *) &lcl, addrlen) < 0) {
	    saved_errno = errno;
	    ff_close(s);
	    freeaddrinfo(server_res);
	    errno = saved_errno;
    ...
    if (ff_connect(s, (struct sockaddr *) server_res->ai_addr, server_res->ai_addrlen) < 0 && errno != EINPROGRESS) {
	saved_errno = errno;
	ff_close(s);
	freeaddrinfo(server_res);
	errno = saved_errno;
