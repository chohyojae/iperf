...
iperf_handle_message_client(struct iperf_test *test)
{
	...
	if ((rval = ff_read(test->ctrl_sck, (char*) &test->state, sizeof(signed char))) <= 0) {
		if (rval == 0) {
			i_errno = IECTRLCLOSE;
			return -1;
		} else {
...
iperf_connect(struct iperf_test *test)
{
	...
	len = sizeof(opt);
	if (ff_getsockopt(test->ctrl_sck, IPPROTO_TCP, TCP_MAXSEG, &opt, &len) < 0) {
		test->ctrl_sck_mss = 0;
	}
	else {
...
iperf_run_client(struct iperf_test * test)
{
...
#ifdef FSTACK
	ff_init("./config.ini", argv_global);
#endif
	...
	while (test->state != IPERF_DONE) {
		...
		result = ff_select(test->max_fd + 1, &read_set, &write_set, NULL, timeout);
